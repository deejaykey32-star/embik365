...
export default function App() {
  ...
  useEffect(() => {
    if (!adminUser && hiddenSectionIds.includes(activeSectionId)) {
      setActiveSectionId('info365');
    }
  }, [adminUser, hiddenSectionIds, activeSectionId]);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/80 backdrop-blur-md animate-fade-in">
          <div className="relative w-full max-w-7xl max-h-[92vh] bg-[#faf7f2] dark:bg-[#0c121e] border border-amber-500/30 rounded-3xl shadow-2xl overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-3 sm:p-6">
              <MediaLibraryViewer initialTab={publicMediaTab} initialPackageId={initialRoute.packageId} readOnly={!adminUser} />
            </div>
          </div>
        </div>
      )}
